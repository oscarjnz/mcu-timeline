...
- **Layout responsive (dos modelos, un solo árbol de componentes):** no son dos páginas separadas, sino un mismo shell (`TimelineApp.tsx`) que se reorganiza con Tailwind en el breakpoint `lg` (1024px). Debajo de `lg`: header apilado + `PhaseNav` (slider horizontal con scroll-snap) + filtros en fila, todo dentro de un bloque `lg:hidden`, tal cual el diseño mobile-first original. Desde `lg`: `DesktopSidebar.tsx` (sticky, columna fija a la izquierda, oculto hasta `lg` con `hidden lg:flex`) con el título, toggles, `ViewModeToggle` y filtros reusados tal cual (sus pills envuelven bien en una columna angosta) más `DesktopPhaseNav.tsx`, una navegación vertical de fases construida aparte (no reusa `PhaseNav`, que está optimizada para touch/scroll horizontal; en sidebar no hace falta scroll). El contenido (`PhaseSection`/`ReleaseOrderList`/`DoomsdayCountdownView`) es compartido sin cambios entre ambos modelos, solo con `lg:max-w-3xl` y tarjetas (`TimelineEntryCard`) con poster y tipografía un poco más grandes desde `lg`. Motivo del cambio (2026-08-25): en laptop el sitio se veía como la columna angosta de mobile centrada con mucho espacio muerto a los lados ("parece casi como vertical/celular"); la sidebar fija resuelve eso sin romper el flujo cronológico vertical, que es correcto conservar porque es un timeline.
